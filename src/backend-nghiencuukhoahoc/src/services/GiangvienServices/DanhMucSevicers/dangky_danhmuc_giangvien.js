...
const dangky_danhmuc_giangvien = async (dataDangKyDanhMuc) => {
  try {
    ...
    console.log("MALOAIDANHMUC", dataDangKyDanhMuc.MALOAIDANHMUC);
    const obj = [];
    ...
    for (let i = 0; i < dataDangKy.length; i++) {
      ...
      if (
        ...
        console.log(
          "Trường hợp Tác giả chịu trách nhiệm đặt biệt DataTyLeTraVe: ",
          DataTyLeTraVe
        );
        if (DataTyLeTraVe.length === 0) {
          ...
          console.log(
            "Trường hợp Tác giả chịu trách nhiệm dự phòng DataTyLeTraVe: ",
            DataTyLeTraVe
          );
        }
      } else {
        console.log("check tacs gia dai dein adasdadaaD: ", TacGiaDaiDien);
        [DataTyLeTraVe] = await pool.execute(
          `
            SELECT 
                ctl.MA_QUY_DOI, 
    ...
    if (DanhMuc[0].GIO_CHUAN !== dataDangKyDanhMuc.SOGIODANHMUCDACHO) {
      console.log(
        "Phát hiện sự khác nhau giữa MALOAIDANHMUC và SOGIODANHMUCDACHO ưu tiên SOGIODANHMUCDACHO: ",
        DanhMuc
      );
      DanhMuc[0].GIO_CHUAN = dataDangKyDanhMuc.SOGIODANHMUCDACHO;
    }
